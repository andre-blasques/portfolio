...
/** Phone mockup with scroll-triggered play (uses actual video file) */
function PhoneScrollVideo({ src, variant = 'dark' }: { src: string; variant?: 'dark' | 'light' }) {
  const videoRef = useRef<HTMLVideoElement>(null)
  ...
  return (
    <div
      ...
      style={{
        width: 224,
        background: isLight ? '#e8e8e8' : '#0a0a0a',
        borderRadius: '2.75rem',
        padding: 10,
        border: `1.5px solid ${isLight ? 'rgba(0,0,0,0.18)' : 'rgba(255,255,255,0.13)'}`,
        boxShadow: isLight
          ? '0 28px 56px rgba(0,0,0,0.2), 0 4px 12px rgba(0,0,0,0.08)'
          : '0 28px 56px rgba(0,0,0,0.65), 0 4px 12px rgba(0,0,0,0.4)',
      }}
    >
      {/* Dynamic island */}
      <div aria-hidden="true" style={{ position: 'absolute', top: 18, left: '50%', transform: 'translateX(-50%)', width: 72, height: 22, background: isLight ? '#e8e8e8' : '#0a0a0a', borderRadius: 12, zIndex: 10 }} />
      {/* Side buttons */}
      <div aria-hidden="true" style={{ position: 'absolute', right: -3, top: 90, width: 3, height: 40, background: isLight ? '#ccc' : '#222', borderRadius: 2 }} />
      <div aria-hidden="true" style={{ position: 'absolute', left: -3, top: 80, width: 3, height: 28, background: isLight ? '#ccc' : '#222', borderRadius: 2 }} />
      <div aria-hidden="true" style={{ position: 'absolute', left: -3, top: 116, width: 3, height: 28, background: isLight ? '#ccc' : '#222', borderRadius: 2 }} />
      {/* Screen */}
      <div style={{ borderRadius: '2.25rem', overflow: 'hidden', aspectRatio: '9 / 19.5', background: '#000' }}>
        <video
          ref={videoRef}
          loop
          muted
...
/** CSS phone mockup wrapping a looping video */
function PhoneVideo({ src, alt, variant = 'dark' }: { src: string; alt: string; variant?: 'dark' | 'light' }) {
  const isLight = variant === 'light'
  return (
    <div
      className="relative mx-auto flex-shrink-0 select-none"
